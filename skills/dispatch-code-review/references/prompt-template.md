...
#### 1. Inspect Changes
1. Obey an explicit Git range in Review Scope exactly. Otherwise inspect unstaged (`git diff`), staged (`git diff --staged`), and untracked (`??`) source/text files, skipping `.scratch/`, generated, vendored, and binary paths. When those current changes are empty, review only the branch merge-base-to-`HEAD` range supplied by the caller. Never substitute `HEAD~1`; an empty default range is `No reviewable changes`.
...
3. Test results are already given to you: read the walkthrough's `## Verification & Validation` for the verify command and its output, and spend your turns on the diff. If that section is empty, holds unfilled template placeholders, or names no command, do not assume the code is tested — budget one turn to run the host verify command yourself, and say so in your report.
...
6. Tool Turn Budget counts every tool call, verification runs included. Complete inspection within it when it names a number; otherwise budget `8 + 2 × <changed files>` turns, counting only files changed since the previous round on a re-review. Spend a tight budget on AST / code-graph queries (`codegraph`, `graphify`) rather than full-file reads. Then emit the report immediately.
