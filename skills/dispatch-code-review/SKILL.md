...
1. **User- or orchestrator-supplied plan/walkthrough** when an explicit path is passed or an orchestrating skill hands one over — skip the script for that kind. When only *one* kind is supplied, resolve the other:
   - **(a) Canonical scratch path** (`.scratch/plan/<yyyy-mm-dd>-<slug>.md` for a plan, `.scratch/plan/<yyyy-mm-dd>-<slug>-walkthrough.md` for a walkthrough): resolve the other kind from the same slug rather than letting the branch derive a different one — otherwise an explicitly named walkthrough gets paired with an unrelated plan, or none:
     ```bash
     node <skills-dir>/dispatch/scripts/resolve-artifact-paths.mjs --kind <other kind> --slug <slug from the supplied filename>
     ```
   - **(b) Non-canonical path**: there is no slug to share — resolve the other kind normally.
   - **(c) Tier handling**: apply branch 2's tier handling to the resolved kind — **the walkthrough is never optional**. A plan that does not exist is passed as `<Plan Path>: None`; a walkthrough that does not exist is authored at the resolved path per [references/walkthrough-template.md](references/walkthrough-template.md) before dispatching. Dispatching without a walkthrough strands `<Walkthrough Path>`, which has no `None` form, leaves the delegate with no test results to read, and leaves Step 3 with no file to log the round into.
...
**Authoring a walkthrough** (either branch, whenever this skill writes one):
- Run the host verify command (from `AGENTS.md` / `CLAUDE.md`) and record the command and its output under `## Verification & Validation` (write `None — no host verify command` when the host names none).
- **Record the result whatever the exit status and dispatch either way** — a red suite is a fact the delegate needs, not a gate: reviewing code that does not yet pass is a normal review. The prompt tells the delegate test results are already given so it spends its turns on the diff.

**Stale-walkthrough guard**: `scratch-existing` matches the branch slug at *any* date, so a walkthrough describing earlier, already-committed work on this branch resolves even for unrelated new work. Compare its `## Changes Made` against the diff actually under review (working tree if dirty/untracked, otherwise merge-base-to-`HEAD` branch diff; see [references/prompt-template.md](references/prompt-template.md) § Inspect Changes). When the walkthrough does not describe that diff, stop and ask whether to overwrite it, review it as-is, or author a new one under a fresh `--slug`. Never silently review new code against a stale walkthrough: its logged rounds also mislabel a first review as `Re-review round <n+1>`, confining the delegate to lines changed since a round that has nothing to do with this work, and its verification section is fed forward as if current.
...
2. **Re-verify** (standalone mode only):
   - Re-run the host verify command (from `AGENTS.md` / `CLAUDE.md`) until green, or until two consecutive runs fail on the same failures.
   - Record the command and its result in `## Verification & Validation`.
   - Record unrelated or pre-existing failures under `## Follow-ups` and surface in the user report rather than chasing them.
...
**Done when:** (standalone only) accepted fixes are applied, undeferred items are recorded under `## Follow-ups`, and the host verify command is green — or has twice failed on the same failures, with those recorded as follow-ups; `## Review Findings & Resolutions` is updated with this round's adjudications (orchestrated: unescalated disputes logged as `[Disputed]`, and rejections of delegate-reported MUST-FIX / SHOULD-FIX under a handed-over `consensus: true` as `[Rejected — pending confirmation]` per `dispatch`'s `references/alignment.md` § Finality); and (standalone only) the user report is delivered with provider prefix.
