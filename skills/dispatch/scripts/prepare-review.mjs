 ...
 * `--kind code` delegates to `prepare-code-review.mjs`. Per-kind differences come from the
...
import {
  ...
  resolveArtifacts,
  resolveSlug,
} from './resolve-artifact-paths.mjs';
...
import { KIND_USAGE, PROMPT_FRAME, REBUTTAL_FRAME, REVIEW_KINDS, reviewKind } from './review-kinds.mjs';
import { prepareCodeReview } from './prepare-code-review.mjs';
...
const USAGE = `Usage: node prepare-review.mjs --kind ${KIND_USAGE} --request <json-file|->\n`;
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (arg === '--kind' && kind === null) kind = argv[++index] ?? '';
    else if (arg === '--request' && request === null) request = argv[++index] ?? null;
    else throw new Error(`unknown or repeated argument "${arg}".\n${USAGE}`);
  }
  if (!kind || !Object.hasOwn(REVIEW_KINDS, kind)) {
    throw new Error(`--kind must be one of ${KIND_USAGE}${kind ? ` (got "${kind}")` : ''}.\n${USAGE}`);
  }
...
function main() {
  ...
  assertPreparationIntegrity(DISPATCH_DIR);
  process.stdout.write(`${JSON.stringify(prepareReview(args.kind, readJsonRequest(args.request)), null, 2)}\n`);
}
...
if (isMainModule(import.meta.url)) {
  try {
    ...
    process.stderr.write(`[prepare-review] ${err.message.replace(/\n+$/, '')}\n`);
    process.exit(1);
  }
}
