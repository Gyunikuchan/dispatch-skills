...
2. Read its single JSON action and preserve `stateFile`. Advance with `--drive --state <file> [--input <json>]` as one background command: it sends any schema-valid reply, runs `launch` and `verify` argv itself, and prints the next action needing you.
