...
Extract approved paths, commands, criterion mappings, and `[GENERATED]` paths from the settled plan. A `verify` action's `argv` makes the driver run, log, and fingerprint every gate command and extract failure identities; never run or parse gate commands yourself. Passing final gates seed a day of per-command baseline reuse on identical content. Reconcile baseline side effects before approval. A nonzero result is `known red — unchanged` only when it matches the accepted baseline identity; otherwise it is a regression.

Evidence must postdate its last mapped mutation. The RED gate runs only red-mapped commands, narrowing a covering suite to its red test files. `[FINAL]` commands skip scoped gates. Scoped gates per attempt and fix round rerun changed-scope commands, deferring criteria only `[FINAL]` commands carry. The final gate, after code review, reruns generators and stale uncovered commands (a covering `npm test` carries its file commands' criteria), and maps observable behavior to owning production paths.

When red criteria exist, the tests-only write subagent receives only their manifest, approved test paths, mapped commands, expected failures, and envelope schema. Admit exactly one valid matrix row per red criterion, then the driver observes the expected failure before production work. A baseline-red collision is admitted only for a criterion marked `Pre-existing: yes`. A quality defect, including a test file that fails to load, permits one bounded same-model repair; another defect or an invalid host RED enters failure disposition. No read review gates RED; code review after production covers test quality.

Failure disposition preserves and fingerprints the tree, then records one ruling: keep for repair, revert attributable paths, inspect first (a later `--run` resumes it), `re-verify` (rerun evidence on the unchanged tree), `retry` (next attempt, ruling as writer context), `red-ruling` (driver-verified carry-over or plan-declared no-failing-state RED), or — only on a user decision — `manual-complete`, ledgering closer, reason, per-criterion and host RED evidence, and repository fingerprint. Caller-dirty, mixed, and drifted paths remain preserved. Only a resolved ruling may close a stable-failure segment.

## Write subagent

A `delegate-write` brief at `promptPath` (sha256 `promptHash`) carries the governing outcome, settled scope, criteria, rules, prior findings, and evidence labelled as evidence; relay the path, not the brief. The writer saves and self-checks its JSON at `expectedEnvelopePath`, then returns status, one-line summary, concerns, and that path in chat. Reply to the driver with only `{"envelopePath":"<expectedEnvelopePath>"}`; the driver binds and validates that file. Rejected or failed launch replies keep their existing cascade behavior.

A configured model array is a launch cascade inside one attempt: attempt each emitted model through the native write-subagent launcher with the identical brief and effort before reporting its failure. Use the exact configured model identifier, or an explicit, verified host-to-config identifier mapping; record the actual launched identifier and failure. A CLI probe cannot establish native-subagent availability. If the launcher cannot select that model and no verified mapping exists, reply `rejected` with the unsupported identifier; a launched subagent that fails replies `failed: {kind, reason}` (availability, authentication, quota, and the other providers.md failure kinds). Either reply advances to the next model without consuming an attempt. Malformed output, verification failure, or defective code from a started subagent follows attempt recovery instead. A partial diff outside the approved scope is restored from the pre-attempt baseline before the next model launches. A finished write that changed other paths asks a per-path `write-scope` ruling; the nearest `skill-hashes.json` above an approved path is auto-approved when it verifies, and production edits from a tests-only write always fail. A terminal kind (`sandbox-unsupported`, `integrity`) or an exhausted cascade ends the run in `done` with `outcome: "failed"` instead of entering failure disposition.
