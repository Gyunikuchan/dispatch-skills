...
- **Output**: `--out <path>` writes the filled prompt (recommended: in OS temp, e.g. `<os-temp>/<date>-<slug>-<kind>-review-prompt[-<target>].md`, keeping `.scratch/` reserved for only the plan and walkthrough — an untracked prompt file inside the workspace trips the delegate's read-only integrity check) and prints the path; omitted, the filled prompt prints to stdout.
...
Standalone mode only (orchestrated mode reports nothing — the orchestrator's own handoff covers it; never echo raw delegate outputs or report bodies verbatim into chat). Prefix with the provider label from the dispatch result, including session deep-link or resume command when available:
