...
import { appendEvent, governingHash, readLedger } from './ledger.mjs';
...
function help() {
  console.log(`Usage:
  node design-amendment.mjs <prepare|activate|reject|abort|recover> --args <json-file|->

JSON args:
  prepare:  {designPath, candidatePath, ledgerPath, baseRevision, affectedIncrements?}
  activate: {designPath, ledgerPath, amendmentId}
  reject|abort: {designPath, ledgerPath, amendmentId}
  recover:  {designPath, ledgerPath}`);
}

if (isMainModule(import.meta.url)) {
  const [action, flag, target] = process.argv.slice(2);
  try {
    if (!action || flag !== '--args' || !target) { help(); process.exit(action ? 1 : 0); }
    const args = JSON.parse(target === '-' ? fs.readFileSync(0, 'utf8') : fs.readFileSync(target, 'utf8'));
    const result = action === 'prepare' ? prepareAmendment(args)
      : action === 'activate' ? activateAmendment(args)
      : action === 'reject' ? rejectAmendment(args)
      : action === 'abort' ? abortAmendment(args)
      : action === 'recover' ? recoverAmendment(args)
      : (() => { throw new Error('Invalid design-amendment action'); })();
