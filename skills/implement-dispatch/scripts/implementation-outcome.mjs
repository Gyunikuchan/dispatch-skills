...
function usage() {
  return `Usage:
  node implementation-outcome.mjs --parse <file|->
  node implementation-outcome.mjs --transition <file|->

--parse validates a file containing the delegate's raw final message.
--transition accepts a JSON object containing resolveImplementationTransition inputs.
`;
}
...
function main() {
  ...
  if (!['--parse', '--transition'].includes(flag) || !file || rest.length > 0) {
    throw new Error(usage().trim());
  }
  const result = flag === '--parse'
    ? parseImplementationOutcome(file === '-' ? fs.readFileSync(0, 'utf8') : fs.readFileSync(file, 'utf8'))
    : resolveImplementationTransition(readJsonInput(file));
