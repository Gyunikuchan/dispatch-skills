...
Each dispatch prints a banner naming its provider, model, and session log path. Tail that log to watch a review in progress (`tail -f "<logFilePath>"`, or `Get-Content -Wait -Tail 30 "<logFilePath>"` in PowerShell). The filled prompt actually sent to the delegate is kept in the repo, written wherever `fill-template.mjs --out` pointed — by convention a review-prompt file beside the plan in your scratch directory. It is useful when a review answers a question you did not think you asked. The session log itself lives under your OS temp directory, not the repo. (A prompt too large for the command line additionally spills to a brief file in its own temp directory; the banner names that file when it happens.)
