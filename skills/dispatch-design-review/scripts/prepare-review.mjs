...
export function designSnapshot(source) {
  return semanticSectionHashes(source, { excludedSections: ['Execution Status'] });
}

function parseRound(roundId, scan) {
  const expected = (scan.rounds.at(-1)?.number ?? 0) + 1;
  if (!roundId) return { roundId: `design-review:R${expected}`, round: expected };
  const match = /^design-review:R([1-9]\d*)$/.exec(roundId);
  if (!match || Number(match[1]) !== expected) {
    throw new Error(`roundId must be design-review:R${expected}.`);
  }
...
function validateRequest(request) {
  assertObjectKeys(request, REQUEST_KEYS, 'design review request', FIELD_HINTS);
...
function validateTargets(entries, roundId, label) {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (entry.roundId !== undefined) {
      if (!/^design-review:R[1-9]\d*$/.test(entry.roundId)) throw new Error(`${label}[${index}].roundId is invalid.`);
      if (roundId !== undefined && entry.roundId !== roundId) throw new Error(`${label}[${index}].roundId is invalid.`);
    }
    if (!/^design-review:[a-z][a-z0-9-]*:\d+$/.test(entry.candidateId ?? '')) throw new Error(`${label}[${index}].candidateId is invalid.`);
...
function resolvePlan(request, repoRoot) {
  ...
  return { slug: result.slug, ...result.design, path: path.resolve(repoRoot, result.design.path) };
}
...
function readCheckpointState(request) {
  ...
  if (state.kind !== 'design') throw new Error('Invocation context kind is not design.');
  return state;
}
...
function observeSettledWrites(state) {
  const artifact = readArtifact(state.artifactPath, { kind: 'design' });
  ...
  const snapshot = designSnapshot(artifact.source);
...
function checkpointPreview(request) {
  ...
  const consensus = evaluateConsensus(readArtifact(state.artifactPath, { kind: 'design' }).source);
...
function checkpoint(request, { now }) {
  ...
  if (JSON.stringify(observed) !== JSON.stringify(declared)) {
    throw new Error(settledWritesMismatch('settledWrites.sections', 'design changes', observed, declared));
  }
...
export function prepareDesignReview(request, {
  ...
  const artifact = readArtifact(resolved.path, { kind: 'design', slug: resolved.slug });
  const lint = lintDesign(artifact.source);
  const defects = lint.diagnostics;
  const warnings = [];
  const persisted = artifact.metadata;
  const native = resolved.tier === 'native' ||
    isNativeArtifactPath(resolved.path, 'design', nativeRoots ? { roots: nativeRoots } : undefined);
  ...
  if (legacyCoverage) {
    return {
      ...
      decision: 'legacy-design-coverage',
      choices: ['overwrite', 'as-is', 'fresh-slug'],
      artifact: { canonicalPath: manifestPath, tier: resolved.tier, slug: resolved.slug },
      freshness: { status: 'legacy', changedSections: [] },
      defects: [...defects, ...warnings].map(defect => ({ ...defect, severity: 'warning' })),
      cleanupPaths: [],
    };
  }
  if (defects.length && !native) {
    return {
      schemaVersion: 1,
      kind: 'design',
      action,
      status: 'decision-required',
      decision: 'design-lint',
      artifact: { canonicalPath: manifestPath, tier: resolved.tier, slug: resolved.slug },
  ...
  const lintWarnings = [
    ...warnings,
    ...(native ? defects.map(defect => ({ ...defect, severity: 'warning' })) : []),
  ];
  const warningScope = lintWarnings.length
    ? `Design lint warnings: ${lintWarnings.map(({ rule, locus, message }) => `${rule} (${locus}): ${message}`).join(' | ')}`
    : '';
  const scope = [derivedScope, request.reviewScope, warningScope].filter(Boolean).join('; ');
  const prompt = reviewMode === 'full'
    ? loadPrompt({
      'Design Path': toManifestPath(reviewPath, repoRoot),
      Requirement: request.requirement ?? artifact.body.match(/^#\s+(.+)$/m)?.[1] ?? 'Review the design',
  ...
  if (request.invocationContext) {
    ...
    if (previous.kind !== 'design' || previous.artifactPath !== path.resolve(resolved.path)) {
      throw new Error('Invocation context does not match the design artifact.');
    }
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify(prepareDesignReview(readJsonRequest(args.request)), null, 2)}\n`);
}
