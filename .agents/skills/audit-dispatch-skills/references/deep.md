...
You audit **one skill end to end**: its `SKILL.md`, references, config, scripts, and the tests that exercise them, plus any test importing its scripts. Two paths depend on which kind of skill you drew:
...
- **`code`**: Readability and architecture — `// SECTION:` grouping, types/tunable constants/main functions hoisted to the top, structure aligned with `skills/dispatch/scripts/runners/claude.mjs`; redundant or dead code (unused variables, uncalled helpers, unreachable branches, superfluous wrappers); redundant comments (restating obvious mechanics or type signatures instead of single-clause non-obvious rationale); helpers duplicated across scripts that belong in `skills/dispatch/scripts/lib/` or `runners/shared.mjs`.
