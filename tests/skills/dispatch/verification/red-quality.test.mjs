...
/** Checks the fixture files in-process: 0 valid, 1 quality defects, 2 unreadable input. */
function run(args) {
  const value = flag => args[args.indexOf(flag) + 1];
  try {
    const load = file => JSON.parse(fs.readFileSync(file, 'utf8'));
    const defects = checkRedQuality(fs.readFileSync(value('--plan'), 'utf8'), load(value('--evidence')), load(value('--red')));
    return defects.length ? { status: 1, stdout: '', stderr: defects.join('; ') } : { status: 0, stdout: JSON.stringify({ status: 'valid' }), stderr: '' };
  } catch (error) {
