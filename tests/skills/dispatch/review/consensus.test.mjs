...
describe('evaluateConsensus gate outcomes', () => {
  it('returns structured enriched findings and rejects non-enriched bullets', () => {
    ...
    const enriched = evaluateConsensus([
      '# Plan',
      '## Review Findings & Resolutions',
      '### Round 1 — Claude',
      `- **Sources:** ${sourceMap}`,
    ...
    assert.deepEqual(enriched.unsettledItems, [{
      key: 'R1-F001',
      id: 'R1-F001',
      severity: 'SHOULD',
    ...
    assert.equal(evaluateConsensus(doc('- **[Disputed]** § A — tag: x → y')).exit, 2, 'non-enriched bullets are rejected by the strict gate');
  });

  it('lists each unsettled line', () => {
    const result = evaluateConsensus(doc('- **[Disputed]** [R1-F037] [MUST] [sources=plan-review:R1:claude:0] § A — tag: x → y', '- **[Rejected — pending confirmation]** [R1-F038] [MUST] [sources=plan-review:R1:claude:0] § B — tag'));
    assert.equal(result.exit, 1);
    assert.match(result.unsettled.join('\n'), /§ A[\s\S]*§ B/);
  });

  it('uses exit 2 for malformed enriched state', () => {
    const malformed = evaluateConsensus(docNoSources(
      '- **[Disputed]** [R1-F001] [MUST] [sources=plan-review:R1:claude:0] § A — tag: x → y',
    ));
    assert.equal(malformed.exit, 2);
    assert.match(malformed.error, /structured source map/);
  });
});
