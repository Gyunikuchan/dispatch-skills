...
} from '../../../../skills/dispatch/scripts/runners/copilot.mjs';
...
describe('copilot-run: runner discovery, reachability & auth classification', () => {
  ...
  describe('auth error classification', () => {
    ...
    it('defaults the direct runner to sandboxed execution with an explicit opt-out', () => {
      assert.equal(parseCopilotArgs(['node', 'runners/copilot.mjs', '-p', 'prompt']).sandbox, true);
      assert.equal(
        parseCopilotArgs(['node', 'runners/copilot.mjs', '--no-sandbox', '-p', 'prompt']).sandbox,
        false,
      );
    });
  });
...
describe('Copilot sandbox downgrade', () => {
  ...
  it('Copilot downgrade reruns the same model unsandboxed, warns on stderr, and flags sandboxDowngraded', async () => {
  ...
  it('Copilot downgrade rerun failure advances the alias cascade keeping the flag without a second downgrade', async () => {
  ...
  it('Copilot downgrade rerun that throws advances to the next alias keeping one warning and the flag', async () => {
  ...
  it('Copilot downgrade rerun that fails sandbox-unsupported again advances aliases without a second downgrade', async () => {
  ...
  it('Copilot downgrade also applies when the sandboxed attempt throws sandbox-unsupported', async () => {
  ...
  it('Copilot sandboxed success omits the downgrade flag and warnings', async () => {
  ...
  it('Copilot explicit sandbox false never requests isolation or flags a downgrade', async () => {
