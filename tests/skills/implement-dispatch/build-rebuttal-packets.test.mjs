...
} from '../../../skills/implement-dispatch/scripts/build-rebuttal-packets.mjs';
...
describe('rebuttal packet builder', () => {
  ...
  it('routes legacy findings conservatively using round-wide affinity', () => {
    const legacy = [
      '# Plan',
      '## Review Findings & Resolutions',
    ...
    const packets = buildRebuttalPackets(legacy, {
      findings: [{
        key: 'legacy:R3:L4',
        orchestratorVerdict: 'disputed',
        counterEvidence: 'The requirement is ambiguous.',
        changedExcerpts: [],
      }],
    });
    assert.deepEqual(packets.map((packet) => packet.sourceKey), [
    ...
    assert.ok(packets.every((packet) => packet.source === null));
  });
