...
import { drive, implementationOutcome, makeGitRepo, PLAN_BODY, writeOutcomeReply, writePlan } from './driver-harness.mjs';
...
export function tierPolicy(fixture, overrides = {}) {
  ...
  return {
    ...
    delegateWrite(action) {
      ...
      return writeOutcomeReply(action, implementationOutcome({ evidence: ids.map(id => `CRITERION ${id} | ${id === 'SC3' ? 'tests/sample.test.mjs' : 'src/app.js'} | delivered value=2`) }));
    },
